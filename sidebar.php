	...
	<?php if ( is_active_sidebar( 'primary_widget_area' ) ) : ?>
		...
		<div id="widget-area" class="widget-area" role="complementary">
			...
			<?php if ( current_user_can( 'manage_options' ) ) : ?>
				<span class="edit-link"><a href="<?php echo admin_url( 'widgets.php' ); ?>" class="badge badge-secondary"><?php _e( 'Edit', 'outandequal' ); ?></a></span><!-- Show Edit Widget link -->
			<?php endif; ?>
		</div><!-- .widget-area -->
	...
	<?php if ( is_archive() || is_single() ) : ?>
		...
		<div class="bg-faded sidebar-nav">
			<div id="primary-two" class="widget-area">
				<?php
					$output = '<ul class="recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							$output .= '<li><h3>' . __( 'Recent Posts', 'outandequal' ) . '</h3></li>';
							while ( $recentposts_query->have_posts() ) :
								...
								$output .= '<h4><a href="' . get_the_permalink() . '" title="' . sprintf( __( 'Permalink to %s', 'outandequal' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' . get_the_title() . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
				...
				<br />
				<ul>
					<li><h3><?php _e( 'Categories', 'outandequal' ); ?></h3></li>
					<?php
					...
					<?php if ( ! is_author() ) : ?>
						<li>&nbsp;</li>
						<li><a href="<?php echo get_the_permalink( get_option( 'page_for_posts' ) ); ?>" class="btn btn-outline-secondary"><?php _e( 'more', 'outandequal' ); ?></a></li>
					<?php endif; ?>
				</ul>
			</div><!-- /#primary-two -->
		</div>
